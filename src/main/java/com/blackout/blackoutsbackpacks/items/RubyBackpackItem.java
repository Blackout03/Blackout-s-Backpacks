...
public class RubyBackpackItem extends BackpackItem {
	...
	public ActionResult<ItemStack> use(World worldIn, PlayerEntity playerIn, Hand handIn) {
		...
		if (!worldIn.isClientSide) {
			...
			if (playerIn.getItemInHand(handIn).getItem() instanceof BackpackItem) {
				NetworkHooks.openGui(serverPlayerEntity, new BackpackItemContainerProvider(handIn, playerIn.getItemInHand(handIn), 12, 9), (buf) -> buf.writeInt(handIn == Hand.MAIN_HAND ? 0 : 1));
